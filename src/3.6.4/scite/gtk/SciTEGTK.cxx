...
FilePath GetSciTEPath (const char *home) {
// Use SciTEGlobal in bins dir if available.

  char *result=(char*)malloc (1000);
  size_t size = readlink( "/proc/self/exe",result,999 );
  struct stat buffer;
  ...
  if (size >0 ){
	...
	size_t found = path.find_last_of ("/");
  	std::string cnf_path = path.substr(0,found) + "/SciTEGlobal.properties";
	path=path.substr(0,found);

	if (stat (cnf_path.c_str(),&buffer)) {
...
FilePath SciTEGTK::GetSciteUserHome() {
	const char *where = getenv("SciTE_HOME");
	std::string tmp =props.GetNewExpandString("SciteUserHome");
	if (!where) {
		where = getenv("HOME");
	}
	if (tmp.length()==0) {
		where = getenv(GUI_TEXT("USERPROFILE"));
	} else {
		//respect changes in property files
		where = tmp.c_str();
	}
	return GetSciTEPath(where) ;
}
